...
export const metadata: Metadata = {
  ...
  description:
    "Începe cu varianta de 7 zile și vezi cum te simți. Preț special: 39 lei.",
};

const BENEFITS = [
  "Plan simplu pentru 7 zile",
  "Mese ușor de pregătit",
  "Fără stres și fără reguli complicate",
];

export default function Offer7DayPage() {
  return (
    <section
      aria-labelledby="offer-7-title"
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-1 items-center px-5 pt-6 sm:px-8 sm:pt-8 lg:pt-4">
        <div className="grid w-full gap-8 lg:grid-cols-[340px_1fr] lg:items-center lg:gap-12">
          <Reveal className="text-center lg:col-start-2 lg:row-start-1 lg:text-left">
            ...
            >
              Nu ești <span className="font-accent text-gold-400">sigură</span> încă?
            </h1>
            <p className="mx-auto mt-3 max-w-lg text-[14px] leading-[1.55] text-cream-100/80 sm:text-[16px] lg:mx-0">
              Începe cu varianta de 7 zile și vezi cum te simți.
            </p>
          </Reveal>
          ...
          >
            <figure className="flex h-full justify-center lg:justify-start">
              <Image
                src="/images/meal-plan-7-day.jpg"
                alt="Plan de nutriție 7 zile — previzualizare"
                width={774}
                height={1200}
          ...
          <Reveal delay={0.1} className="lg:col-start-2 lg:row-start-3">
            <div>
              ...
              <ul className="mx-auto grid max-w-xs gap-2 text-[14px] leading-[1.5] text-cream-100/85 sm:max-w-sm sm:text-[15px] sm:gap-3 lg:mx-0 lg:max-w-none">
                {BENEFITS.map((line) => (
          ...
          <Reveal delay={0.15} className="lg:col-start-2 lg:row-start-4">
            <article className="border border-cream-50/10 bg-olive-900/40 p-5 sm:p-6">
              <p className="text-[14px] leading-[1.55] text-cream-100/85 sm:text-[15px]">
                Este varianta perfectă dacă vrei să testezi fără să te
                complici.
              </p>
            </article>
          </Reveal>
